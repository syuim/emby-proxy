...
- `/img`：通用图片代理，无鉴权
- `/douban/...`：豆瓣 Stremio addon 反代（`http://rn.127315.xyz:31001` 直连，不依赖 fw-douban.laoz.org），无鉴权。UA 透传保留 forward 行为；注入 `X-Forwarded-Host/Proto` 使 addon 生成的图片 URL 闭环到 `/douban/image-proxy`（响应 JSON 再前缀改写兜底）；仅 `/image-proxy` 与 `/assets/` 走 CF 边缘缓存，catalog 等 JSON 不缓存（UA 不进 cache key，forward UA 的 `tmdb:` ID 响应会污染普通缓存）
