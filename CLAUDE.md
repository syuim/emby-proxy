...
# Go 节点
cd proxy-go && go test ./...
cd proxy-go && go build -o emby-proxy .

# CF Worker
cd cf-worker && npm install
cd cf-worker && npx tsc --noEmit
cd cf-worker && npx wrangler dev          # 本地 dev，读 .dev.vars
cd cf-worker && npx wrangler deploy       # 部署到 CF
```

改完代码必跑：`go test ./...`（动了 proxy-go/）+ `npx tsc --noEmit`（动了 cf-worker/）。

## CI/CD

`cf-worker/**` 任何改动 push 到 `main` → 触发 `.github/workflows/deploy-cf-worker.yml` → `cloudflare/wrangler-action@v3` 部署到 Cloudflare。
...
- `Dockerfile` — `golang:1.22-alpine` 多阶段构建 → `alpine:3.19`，暴露 `:8080`，healthcheck `/__health`
...
| 变量 | 说明 | 必填 |
|---|---|---|
| `EMBY_SYNC_TOKEN` | 校验 cf-worker 推送鉴权（与 cf-worker secret 同值） | 是 |
...
代码 push 到 `origin/main` 后，SSH 到 dash 拉取并编译：
...
ssh -i ~/.ssh/syu_vps -p 22222 root@dash.127315.xyz \
  "cd /root/docker && git clone git@github.com:syuim/emby-proxy.git && \
   ...
   echo 'EMBY_SYNC_TOKEN=<同 cf-worker secret>' > .env && \
...
**自动部署**：push 到 `origin/main` 触发 GitHub Actions（仅 `cf-worker/` 改动），用 `wrangler-action@v3` 调 `npx wrangler deploy`。
...
1. 本地提交并 push 到 `origin/main`
2. cf-worker 改动 → CI 自动部署到 Cloudflare（约 1 分钟）
3. proxy-go 改动 → SSH dash 执行 `git pull && cd proxy-go && docker compose build && docker compose up -d`
4. 验证：
   - 节点：`curl http://dash.127315.xyz:8080/__health` 返回 `ok`
   - Worker：`curl https://<worker-domain>/__health` 返回 `ok`
   - 端到端：`curl -I https://<worker-domain>/<emby_name>/` 应返回 307 Location 指向某节点
...
- **故障转移**：emby 指定 node_id 不健康 → router 从其他节点中**随机**挑一个健康的 → 全不健康兜底原 node_id
...
1. 改 `src/*.ts`，本地 `npm run dev` 或 `npx wrangler dev` 烟雾测试
2. `npx tsc --noEmit` 类型检查
3. 提交 + push → CI 自动部署

### 修改 proxy-go

1. 改 `proxy-go/*.go`，`go test ./...` 跑测试
2. 协议变更需保持 `/admin/sync` payload 向后兼容（path_prefix 字段名不能动）
3. 提交 + push → SSH dash 拉取重建

### 添加新节点

1. SSH 新机器，`docker run` 或 clone 仓库 + `docker compose up -d` 起 emby-proxy
2. 在 cf-worker 管理 UI Nodes 页加节点（name + public_url）
3. 等 1 分钟看 health 状态变绿，applied_version 与 cf-worker 一致
...
常用诊断：
- 节点日志：`ssh dash 'docker logs --tail 100 emby-proxy-emby-proxy-1'`
- Worker 日志：`cd cf-worker && npx wrangler tail --format pretty`
- KV 内容：`cd cf-worker && npx wrangler kv key list --binding=EMBY_KV`
